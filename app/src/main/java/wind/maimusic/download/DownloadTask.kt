...
//                Log.e("JG","文件下载储存路径：${Constant.STORAGE_SONG_FILE}")
                ...
                if (contentLength == 0L) {
                    return Consts.TYPE_DOWNLOAD_FAILED
                } else if (contentLength == downloadedLength) {
                    return Consts.TYPE_DOWNLOADED
                }
                listener.onStart()
                ...
                while (inputStream!!.read(b).also { len = it } != -1) {
                    if (mIsCancel) {
                        return Consts.TYPE_DOWNLOAD_CANCELED
                    } else if (mIsPause) {
                        return Consts.TYPE_DOWNLOAD_PAUSED
                    } else {
                        total += len
                        raf?.write(b,0,len)
                ...
                response.body?.close()
                return Consts.TYPE_DOWNLOAD_SUCCESS
            } catch (e:Exception) {
                e.printStackTrace()
            } finally {
                try {
    ...
    override fun onPostExecute(result: Int?) {
        result?.let {
            when(it) {
                Consts.TYPE_DOWNLOAD_SUCCESS->listener.onSuccess()
                Consts.TYPE_DOWNLOAD_FAILED->listener.onFailed()
                Consts.TYPE_DOWNLOAD_PAUSED->listener.onPaused()
                Consts.TYPE_DOWNLOAD_CANCELED->listener.onCancel()
                Consts.TYPE_DOWNLOADED->listener.hasDownloaded()
            }
        }
    }
    fun pauseDownload() {
